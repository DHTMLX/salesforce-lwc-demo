...
- **DHTMLX Kanban** - Task board with drag-and-drop functionality  
- **DHTMLX Gantt** - Project management with task dependencies
- **DHTMLX Scheduler** - Event calendar and timeline management
...
**[Click here to open the Live Demo >>>](https://dhtmlx-dev-ed.develop.lightning.force.com/lightning/n/Kanban)**
...
5. **Create a Scratch Org and Assign its Alias**
   Now, we will create a temporary "scratch" org for the demo. We must give it a nickname (an alias) so we can easily send commands to it.
    ```sh
